...
## 🌐 Aplicação Web (Front/Back Office)

Inclui um front office com landing page, registo de acesso e login, e um backoffice para controlo, estatísticas, criação de roadmaps e registo de insights.

### Multi-lingue

Interface disponível em: Português (PT-PT), Inglês, Espanhol, Francês e Alemão.
...
- `/register` Registo
- `/login` Login
- `/backoffice` Backoffice (protegido)
...
linkedin-optimizer/
├── app/
│   ├── main.py                   # FastAPI (front/back office)
│   ├── db.py                     # SQLite + queries
│   ├── security.py               # Password hashing + sessão
